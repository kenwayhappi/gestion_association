<?php
    ...
    $res = mysqli_fetch_row(mysqli_query($conn,"select count(*) from seance  where date_seance >= NOW() and id_association='$complet'"));
    if($res[0] != 0){
        echo "<script>
        alert(\" Une seance deja lancer\");
...
<body>
    <main>
        ...
        <ul class="breadcrumbs">
            <li><a href="cotisation.php">seance</a></li>
            <li class="divider">/</li>
...
if(isset($_POST['submit'])) 
{
    ...
    $heure=$_POST['heure'];
    $raison=$_POST['raison'];
    $id_membre=mysqli_query($conn,"SELECT * FROM membres where id_association='".$_SESSION['id_association']."'");
        ...
        VALUES ('$id_association','$lieu','$date_seance','$heure','$raison')";
                ...
                if ($res){
                    mysqli_query($conn,$reqadd);
                    echo '<div class="alert alert-success" role="alert"><strong>Cotisation creer avec succès!</strong></div>';
                    /*$reqmod="UPDATE association SET 
                    nbr_membre++;
                    WHERE id_association='$membre_asso'";*/
                }
                else{
                    echo '<div class="alert alert-danger" role="alert"><strong>Echec de la création!</strong></div>';
                }}
?>
                </form>
            </div>
